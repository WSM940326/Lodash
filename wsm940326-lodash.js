 ...
 */
  ...
  forEach: function (array, comm) {
    for (var i = 0; i < ary.length; i++) {
      if (comm(ary[i], i, ary) === false) {
        break
      }
    }
    return ary
   ...
   * javascript comment 
   * @Author: flydreame 
   * @Date: 2018-07-19 23:01:22 
   * @Desc:  
   */

  groupBy: function (ary, predicate) {
    var groupBy = (ary, predicate) => ary.reduce((map, item, key) => (key = predicate(item), key in map ? map[key].push(item) : map[key] = [item], map), {})
  },
